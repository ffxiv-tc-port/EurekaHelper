...
internal static class Loc
{
	...
	private static readonly Dictionary<string, string> UiZhTw = new Dictionary<string, string>
	{
		...
		["Occupied"] = "有人",
		["Empty"] = "無人",
		["Tower Status Unknown"] = "狀態未知",
		["Ovni cannot respawn while the tower is occupied, per the community tracker."] = "根據社群追蹤站資料，塔內有人時未確認飛行物體不會刷新。",
		["Spawned"] = "已出現",
		["Spawned at"] = "出現於",
		["Pull Timer:"] = "開打倒數：",
		["Predicted:"] = "預測出現：",
		["Insufficient data"] = "資料不足，無法預估",
		["Upcoming Schedule:"] = "主催排班表：",
		["No upcoming schedule"] = "尚無排班",
		["Treasure Hunt"] = "尋寶",
		...
		["Direction"] = "方位",
		["Distance Tier"] = "距離等級",
		["Click to place a map flag here."] = "點選在此設定地圖旗標。",
		["Set Map Flag"] = "設定地圖旗標",
		...
		["Click to set a flag marker"] = "點選設定地圖旗標",
		["Can Mutate"] = "可變異",
		["Can Adapt"] = "可適應",
		...
		["? (not measured)"] = "？（尚未實測）",
		["(range ?)"] = "（範圍未知）",
		["This aggro type has no measured radius yet, so nothing is drawn for it in-game by default. Lock the mob and measure it below, or turn on the \"unmeasured\" marker in Configuration."] = "這個仇恨類型還沒有實測半徑，所以預設在遊戲畫面上完全不會畫出來。可以鎖定該怪在下方實測填入，或到設定頁開啟「範圍未知標記」。",
		["Mark aggro types whose range isn't measured yet"] = "標示範圍尚未實測的仇恨類型",
		["Magic/Blood aggro types ship with radius 0, which Splatoon draws as nothing at all - on screen that looks exactly like \"this mob is harmless\". Turn this on to draw a small fixed-size ring instead, meaning \"this type is dangerous but its range is unknown\" rather than pretending the range is that size."] = "魔法感知／血量感知這兩型出廠半徑是 0，而半徑 0 在 Splatoon 上等於完全不畫 —— 畫面上看起來就跟「這隻沒有威脅」一模一樣。開啟後會改畫一個固定大小的小圈，意思是「這一型有威脅，但範圍未知」，而不是假裝範圍就是那麼大。",
		["+/- {0:0}y"] = "±{0:0} 碼",
		["How wide the current distance tier is. Collect more hints (use another Lucky Carrot) to narrow it down."] = "目前距離等級的區間寬度。多收集幾則提示（再用一次幸運胡蘿蔔）就會縮小。",
		["+/- ? (no upper bound)"] = "±？（無上限）",
		["The latest hint's distance tier has no known upper limit, so the distance really is unknown - the treasure could be much further than the drawn lines suggest. Only the direction is meaningful right now."] = "最新一則提示的距離等級沒有已知上限，所以距離**真的是未知的** —— 財寶可能比畫出來的線段還要遠很多。現階段只有方向是可信的。",
		["({0} hint(s))"] = "（已收集 {0} 則提示）",
		["Hazards"] = "隱藏危險",
		["Hazard marking is turned off."] = "隱藏危險標示目前是關閉的。",
		["Mark hidden traps / portals (Eureka & Baldesion Arsenal)"] = "標示隱藏陷阱／傳送門（優雷卡與兵武塔）",
		["Requires the Splatoon plugin. Remembers where traps/portals were found and re-draws them on later visits as \"possible\" until you actually see them again.\nShips with NO built-in location data - you classify what you find yourself in the Hazards tab."] = "需要安裝 Splatoon 外掛。會記住陷阱／傳送門出現過的位置，下次再進來時先以「可能有」畫出來，直到你真的又看到它才轉成「已確認」。\n出廠不含任何內建座標資料 —— 要由你自己在「隱藏危險」分頁把遇到的物件分類。",
		["Marks hidden traps and portals in Eureka / the Baldesion Arsenal in two states: \"possible\" (found here on an earlier visit, not visible right now) and \"confirmed\" (visible right now)."] = "以兩種狀態標示優雷卡／兵武塔裡的隱藏陷阱與傳送門：「可能有」（以前來的時候在這裡出現過，但現在看不到）與「已確認」（此刻就看得到）。",
		["This ships with NO built-in locations. Hidden objects have no name in the game's own data, so there is no offline table that says which object is a trap - you classify what you run into below, and only classified objects start accumulating positions."] = "本功能不含任何內建座標。隱藏物件在遊戲自己的資料裡就是沒有名字的，因此不存在一份可以離線查出「哪個物件是陷阱」的表 —— 請在下方把實際遇到的物件分類，只有分類過的物件才會開始累積座標。",
		["Splatoon: Not Connected (nothing will be drawn in-game)"] = "Splatoon：未連線（遊戲畫面上不會畫出任何東西）",
		["Current area:"] = "目前位置：",
		["Map {0}"] = "地圖 {0}",
		["This map:"] = "這張地圖：",
		["nothing classified yet - unknown, not \"none\""] = "尚未分類任何物件 —— 是「不知道」，不是「沒有」",
		["Until you classify at least one object as a trap or portal, this plugin has no idea whether this map has any. That is different from knowing there are none."] = "在你至少把一個物件分類成陷阱或傳送門之前，本外掛無從得知這張地圖上有沒有。這跟「已確認沒有」是兩回事。",
		["{0} confirmed"] = "已確認 {0} 處",
		["/ {0} possible"] = "／可能有 {0} 處",
		["\"Confirmed\" = visible in the object table right now (drawn solid). \"Possible\" = recorded on an earlier visit but not visible right now (drawn dim)."] = "「已確認」＝此刻真的在物件表裡看得到（畫成實線）。「可能有」＝以前來的時候記錄過，但現在看不到（畫成淡線）。",
		["Discovered objects (classify here)"] = "已發現的物件（在此分類）",
		["Every event object seen in Eureka this session, one row per Data ID. Set the ones that turned out to be traps or portals; everything else can stay unclassified."] = "本次遊玩在優雷卡看到的所有事件物件，一個 Data ID 一列。把實際證實是陷阱或傳送門的標起來即可，其餘的維持未分類就好。",
		["Nothing discovered yet - walk around Eureka with this enabled."] = "尚未發現任何物件 —— 開著這個功能在優雷卡走動即可。",
		["Data ID"] = "Data ID",
		["First Seen"] = "首次發現",
		["Classification"] = "分類",
		["(no name)"] = "（無名稱）",
		["Hidden objects genuinely have no name in the game data - this is expected, not a read failure."] = "隱藏物件在遊戲資料裡本來就沒有名字 —— 這是正常現象，不是讀取失敗。",
		["Clear discovered list"] = "清除已發現清單",
		["Only clears the discovery list; your classifications and recorded positions are kept."] = "只會清掉發現清單；你的分類設定與已記錄的座標都會保留。",
		["Recorded positions"] = "已記錄的座標",
		["{0} recorded position(s)."] = "已記錄 {0} 個座標。",
		["Clear positions"] = "清除座標",
		["Area"] = "區域",
		["Type"] = "類型",
		["Seen"] = "看到次數",
		["First seen {0}, last seen {1}"] = "首次發現 {0}，最後一次 {1}",
		["Data file:"] = "資料檔：",
		["HazardKind.Unclassified"] = "未分類",
		["HazardKind.Trap"] = "陷阱",
		["HazardKind.Portal"] = "傳送門",
		["HazardKind.Ignored"] = "忽略"
	};
